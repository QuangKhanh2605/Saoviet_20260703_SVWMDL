...
#define T_REG_OFF  0     /*!< Log without bitmask */
    ...
    #define APP_PPRINTF(...)  do{ } while( UTIL_ADV_TRACE_OK \
                              != UTIL_ADV_TRACE_COND_FSend(DBLEVEL_M, T_REG_OFF, TS_OFF, __VA_ARGS__) ) /* Polling Mode */
    #define APP_TPRINTF(...)   do{ {UTIL_ADV_TRACE_COND_FSend(DBLEVEL_M, T_REG_OFF, TS_ON, __VA_ARGS__);} }while(0); /* with timestamp */
    #define APP_PRINTF(...)   do{ {UTIL_ADV_TRACE_COND_FSend(DBLEVEL_M, T_REG_OFF, TS_OFF, __VA_ARGS__);} }while(0);
...
typedef enum
{
    _CONNECT_DATA_MAIN = 0,		
    _CONNECT_DATA_BACKUP,	
    _CONNECT_FTP_UPLOAD,	
    _CONNECT_FTP,		       
    _CONNECT_HTTP , 
}sKindConnect;
...
uint8_t     UTIL_Convert_IP_To_Buff (char *sIP, uint8_t *aIP_BUF);
