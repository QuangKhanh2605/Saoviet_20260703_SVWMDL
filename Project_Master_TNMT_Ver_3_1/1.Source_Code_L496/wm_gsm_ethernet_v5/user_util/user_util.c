...
sCommonVariable	UTIL_var = 
{
    .ModePower_u8   = _POWER_MODE_ONLINE,             
    .ModeConnNow_u8 = _CONNECT_DATA_MAIN,
};
...
void UtilIntToStringWithScale (int64_t number, char *pdata, uint8_t scale)
{
    float val = 0;
    
    if (scale >= 0xFB) {
        val = ((float) number) * Convert_Scale(0xFF - scale + 1);
        UtilFloatToString(val, pdata, 0);
    } else {
        val = ((float) number) / Convert_Scale(scale);
...
uint8_t UTIL_Convert_IP_To_Buff (char *pIP, uint8_t *aIP_BUF)
{
    uint8_t aIP[6] = {0};
    char aTEMP[10] = {0};
    char *sNext = pIP;
    ...
    while (i < 4)
    {
        ...
        aIP[i] = (uint16_t) UtilStringToInt(aTEMP);
        i++;
    }
...
/*
    ...
    * return - 0xFFFF if not find number
             ...
             - *Pos end
*/
int64_t UTIL_Get_Num_From_Str(sData *strRecei, uint16_t *Pos)
{
    int64_t Value = -1, i = 0;
...
double UtilStringToDouble(const char* str) 
{
...
{
    sprintf(buffer, "%.*lf", decimals, number);
}



int16_t UTIL_Cut_String (char *src, char *dst, char c, uint16_t max)
{
    uint16_t i = 0;
...
/*
    Func: chuyen doi phan thap phan vao so float
*/
float UTIL_Convert_Decimal_Element (int dec)
{
    float result = dec;
...
/*
    Func: decode string factor sett: 0.001  1.02 ....
*/
            
uint8_t UTIL_Count_Decimal_Places(char *str) 
{ 
    char *decimalPoint = strchr(str, '.'); 
    if (decimalPoint == NULL) {
        decimalPoint = strchr(str, ','); 
        if (decimalPoint == NULL) {
...
// Kiểm tra nếu ký tự nằm trong phạm vi từ '0' đến '9'
...
char *UTIL_Get_strNum (const char *input, char *output, uint16_t max) 
{
    int i = 0, found = 0;
    
    while (*input) 
    {
        if (UTIL_IsDigit(*input)) { // Nếu ký tự là số
            found = 1;
            
            output[i++] = *input; // Thêm ký tự vào chuỗi output
            if (i >= (max - 1)) {
                return NULL;
            }
        } else if (found) { 
    ...
    if (found) { // Nếu đã bắt đầu chuỗi số và gặp ký tự không phải số
        output[i] = '\0'; // Kết thúc chuỗi bằng ký tự null
        return (char *) input;
    }
...
void NTP_To_ST_TIME(uint32_t ntp_sec, ST_TIME_FORMAT *sTime)
{
    ...
    /* NTP -> Unix */
    unix_sec = ntp_sec - 2208988800UL;

    /* GMT+7 */
//    unix_sec += 7UL * 3600UL;

    days = unix_sec / 86400UL;
    rem  = unix_sec % 86400UL;

    /* Time */
    sTime->hour = rem / 3600UL;
    rem %= 3600UL;
    ...
    while(1)
    {
        ...
        days_year = IsLeapYear(year) ? 366 : 365;
    ...
    */
    {
        ...
        total_days = unix_sec / 86400UL;

        sTime->day = ((total_days + 3) % 7) + 1;
    }
