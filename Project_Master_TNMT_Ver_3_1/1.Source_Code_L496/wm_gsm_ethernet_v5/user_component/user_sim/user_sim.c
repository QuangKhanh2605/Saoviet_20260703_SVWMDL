...
sEvent_struct sEventSim[] =
{
    ...
    { _EVENT_SIM_UART_RECEIVE, 		0, 0, 5, 				fevent_sim_uart_receive_handler }, 
...
#define SIM_DMA_LENGTH          50
#define SIM_UART_MAX_LENGTH     1500

uint8_t UartSimRxByte = 0;
static uint8_t uartSimBuffReceive[SIM_UART_MAX_LENGTH] = {0};
sData 	sUartSim = {&uartSimBuffReceive[0], 0};

#if (UART_SIM_MODE == UART_SIM_DMA)
...
uint8_t Sim_Task(void)
{
	...
	for (uint8_t i = 0; i < _EVENT_SIM_END; i++)
	{
...
#endif
    HAL_NVIC_DisableIRQ(USART_SIM_IRQn);
}
    ...
    *(sUartSim.Data_a8 + sUartSim.Length_u16++) = UartSimRxByte;
...
#else
    uint16_t NbByteRec = 0;
    uint16_t OldPosData = OldPosDMASim;
       
    if (uart_dma_sim.Init.Mode == DMA_NORMAL)   //sua o day
    {
        if ( (sUartSim.Length_u16 + Size) >= SIM_UART_MAX_LENGTH )
            sUartSim.Length_u16 = 0;
        
        UTIL_MEM_cpy(sUartSim.Data_a8 + sUartSim.Length_u16, aUART_RX_DMA_2, Size);
    ...
    } else
    {
        ...
        if (Size > OldPosData)
        {
            ...
            if ( (sUartSim.Length_u16 + NbByteRec) >= SIM_UART_MAX_LENGTH )
                sUartSim.Length_u16 = 0;
        
            UTIL_MEM_cpy(sUartSim.Data_a8 + sUartSim.Length_u16, &aUART_RX_DMA_2[OldPosData], NbByteRec);
        ...
        } else
        {
            ...
            //Kiem tra length cua buff nhan
            if ( (sUartSim.Length_u16 + Size + NbByteRec) >= SIM_UART_MAX_LENGTH )
                sUartSim.Length_u16 = 0;
...
static uint8_t fevent_sim_at_send_handler(uint8_t event)
{
    ...
    if (sSimVar.IsPause_u8 == FALSE)
    {
        ...
        if (sim_step < SIM_STEP_END) {
            ...
            if ( (sATCmd->at_string != NULL) || (sATCmd->at_response != NULL) ) {
            ...
            if (sATCmd->at_response == NULL) {
                if (sATCmd->callback_success != NULL)
                    sATCmd->callback_success (&sUartSim, 0);
...
static uint8_t fevent_sim_at_send_ok_handler(uint8_t event)
{
	...
	if (sSimVar.IsMatch_u8 == TRUE)
	{
        ...
        if (SIM_GET_DELAY_AT(StepNow) != 0) {
            sEventSim[_EVENT_SIM_AT_SEND].e_period = SIM_GET_DELAY_AT(StepNow);
            fevent_enable(sEventSim, _EVENT_SIM_AT_SEND);
        } else {
            fevent_active(sEventSim, _EVENT_SIM_AT_SEND);
        } 
	} else {
		/*=== Neu Attach can retry lai 20 lan=  | Neu Open TCP hay lenh khac 
          ========Cho sang timeout====*/
		if ((StepNow == SIM_STEP_ATTACH) && (cAttach_u8 < SIM_MAX_CGATT)) {
            cAttach_u8++;
            sSimVar.Retry_u8 = 0;
            UTIL_TIMER_SetPeriod(&TimerSendTimeout, SIM_CGATT_TIMEOUT); 
            UTIL_TIMER_Start(&TimerSendTimeout);
		} else {  
...
static uint8_t fevent_sim_at_send_timeout_handler(uint8_t event)
{
    ...
    uint8_t MAX_RETRY = SIM_CMD_RETRY;
    ...
    if ((StepNow == SIM_STEP_FIRST)
        || (StepNow == SIM_STEP_SIM_READY) ){
        //Retry 5 lan: moi lan 1s
        MAX_RETRY = SIM_CMD_RETRY_2;
    }
    ...
    if ( (sSimVar.Retry_u8 >= MAX_RETRY) || (sSimCommVar.State_u8 == _SIM_OPENED_TCP) )
    {
        sSimVar.Retry_u8 = 0;
...
uint8_t Sim_Push_Block_To_Queue(uint8_t *block_sim_step)
{
    uint8_t length = strlen( (char *) block_sim_step);
...
uint8_t Sim_Check_Response(uint8_t sim_step) 
{
    ...
    sData   strCheck;
    sData strSource = {sUartSim.Data_a8, sUartSim.Length_u16};
    ...
    if (sim_step < SIM_STEP_END)
	{
        ...
        if (sATCmd->at_response != NULL) {
            PosFind = Find_String_V2(&strCheck, &strSource);
            if (PosFind >= 0) {     
                PosFind += strCheck.Length_u16;
                sSimVar.IsMatch_u8 = sATCmd->callback_success (&strSource, PosFind);
                fevent_active(sEventSim, _EVENT_SIM_AT_SEND_OK);  
    ...
    if (SIM_IS_STEP_CHECK_URC (sim_step) == TRUE) {
        for (i = SIM_URC_FIRST; i < SIM_URC_END; i++) {
            ...
            if (PosFind >= 0) {
                PosFind += strCheck.Length_u16;
                sSIM_URC[i].callback_success(&strSource, PosFind);
            }
        }
    }
    
    if (sUartSim.Length_u16 > strSource.Length_u16) {
        i = sUartSim.Length_u16 - strSource.Length_u16;
        UTIL_MEM_cpy(sUartSim.Data_a8, sUartSim.Data_a8 + strSource.Length_u16 - 1, i);
        sUartSim.Length_u16 = i;
        *(sUartSim.Data_a8 + sUartSim.Length_u16) = 0x00;
...
*/
uint8_t Sim_Ctrl_DTR_Pin (void)
{
    if (sSimVar.Retry_u8 == 0) {  
        sSimVar.Retry_u8++;
        
        if (sSimVar.pSim_Ctrl_DTR != NULL) {
            sSimVar.pSim_Ctrl_DTR ();
            return true;
        }
    }
...
/*
    if CGATT -> timeout de check lai
*/
void Sim_Process_Attach (void)
{
    if (Sim_Get_Step_From_Queue(0) == SIM_STEP_ATTACH) {
        fevent_active(sEventSim, _EVENT_SIM_AT_SEND_TIMEOUT);
    }
}
