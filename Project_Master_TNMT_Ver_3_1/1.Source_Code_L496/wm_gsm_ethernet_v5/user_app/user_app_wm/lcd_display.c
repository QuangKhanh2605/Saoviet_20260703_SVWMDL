...
sOjectInformation   sLCDObject[] = 
{
    ...
    {   __SD_FREE,          "SD_FR: ",          NULL,   _DTYPE_I32,      0,      " (MB)",   7,  0,  0x00,    _LCD_SCREEN_1    },
...
//    {   __SC1_ID_DCU,       NULL,               NULL,   _DTYPE_STRING,   0,      NULL,      2,  20, 0x00,    _LCD_SCREEN_CM44    },
    ...
    {   __CHANGE_PASS_1,      "Enter New Password",   NULL,   _DTYPE_STRING,   0,      NULL,      2,  14, 0x00,    _LCD_SCR_CHANGE_PASS    },
    {   __CHANGE_PASS_2,      NULL,               NULL,   _DTYPE_STRING,   0,      NULL,      3,  48, 0x00,    _LCD_SCR_CHANGE_PASS    },
...
void Display_Init (void)
{
    ...
    sATCmdList[_GET_PASSWORD].CallBack = AT_CMD_SET_Get_Password;
    sATCmdList[_SET_PASSWORD].CallBack = AT_CMD_SET_Set_Password;  
#endif
...
//    glcd_init();
    ...
    sLCDObject[__SD_FREE].pData = &SD_Free_i32;  
...
/*
    Get data vÃ o 1 con tro: biet kieu du lieu o do
*/
...
/*===========================Hanlde Password============================*/
uint8_t Save_Password(uint8_t *aData)
{
    ...
    memcpy(aPASSWORD, aData, 4);
    Save_Array(ADDR_SAVE_PASSWORD, aPASSWORD, 4);
    return 1;
#else 
    return 0;
#endif
...
void Init_Password(void)
{
    ...
    if(*(__IO uint8_t*)(ADDR_SAVE_PASSWORD) != FLASH_BYTE_EMPTY)
    {
        for(uint8_t i = 0; i < 4; i++)
        {
            key = *(__IO uint8_t*)(ADDR_SAVE_PASSWORD + 2 + i);
            if(key >= '0' && key <= '9')
            {
                aPASSWORD[i] = key;
                result--;
...
void AT_CMD_SET_Set_Password(sData *strRecei, uint16_t Pos)
{
    uint8_t aData[4] = {0};
    for(uint8_t i = 0; i<4; i++)
    {
        if(strRecei->Data_a8[i] != NULL)
            aData[i] = strRecei->Data_a8[i];
        else 
            break;
    }
    
    if(Save_Password(aData) == 1)
        Modem_Respond_Str(PortConfig, (char*)"OK", 0);
    else
        Modem_Respond_Str(PortConfig, (char*)"ERROR", 0);
}

void AT_CMD_SET_Get_Password(sData *strRecei, uint16_t Pos)
{
    Modem_Respond_Str(PortConfig, (char*)aPASSWORD, 0);
}
