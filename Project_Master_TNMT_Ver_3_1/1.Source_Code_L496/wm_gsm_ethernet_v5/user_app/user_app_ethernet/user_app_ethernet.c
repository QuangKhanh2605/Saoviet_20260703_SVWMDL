...
{    
    ...
    { _EVENT_ETH_RUN_DHCP, 		    0, 0, 10, 			_Cb_Running_DHCP },   //Must set period = 0
    ...
    { _EVENT_ETH_MODBTCP_CLIENT,    0, 0, 10,           _Cb_ModbTCP_Client},
...
{   
    ...
    if(sAppEthVar.isDHCP == true)
    {
        if (gWIZNETINFO.ipmode >= NETINFO_DHCP_V4) {
            if (DHCPipver == AS_IPV4)
                DHCPv4_run();
            else 
                DHCP_run(&gWIZNETINFO); 
        }
    ...
    //Tat event duoi
    fevent_disable(sEventAppEth, _EVENT_ETH_DNS_PROCESS);
    fevent_disable(sEventAppEth, _EVENT_ETH_MODBTCP_CLIENT);
...
//                }
		...
		default:    //success    
            ...
            fevent_active(sEventAppEth, _EVENT_ETH_MODBTCP_CLIENT);
            ...
            sAppEthVar.isDHCP = true;
	}

    fevent_active(sEventAppEth, event);
    return 1;
}
...
static uint8_t _Cb_DNS_Process (uint8_t event)
{
	...
	switch (step)
	{
		...
		default:
            ...
            if (ret == 1) {
                char aTEMP[100] = {0};
                sprintf (aTEMP, "u_app_eth: DNS Success %s: %d.%d.%d.%d\r\n",(uint8_t*)sIP, pIP[0], pIP[1], pIP[2], pIP[3]);
...
{ 
    ...
    switch (UTIL_var.ModeConnNow_u8)
    {
        ...
        case _CONNECT_DATA_BACKUP:
            ...
            SockCtrlState = AppEth_Socket_Control (SOCKET_MQTT, sAppEthVar.sServer.aIP, *sAppEthVar.sServer.Port_u16);
            ...
            if (SockCtrlState == SOCK_ESTABLISHED) {
                ...
                if ( (!sAppEthVar.CtrSockConnected_u8) || (sAppEthVar.Status_u8 < _ETH_TCP_CONNECT) )
                {
                    ...
                    char dat[100]={0};
                    sprintf(dat,"Sock state: %d AnyPortCtrl: %d IP: %s Port: %d\r\n", getSn_SR(SOCKET_MQTT), sAppEthVar.AnyPortCtrl, sAppEthVar.sServer.sIP, *sAppEthVar.sServer.Port_u16);
        ...
        case _CONNECT_FTP:
            ...
            SockCtrlState = AppEth_Socket_Control (CTRL_SOCK, sAppEthVar.sUpdateVar.sServer.aIP, *sAppEthVar.sUpdateVar.sServer.Port_u16);
...
//                    AnyPortCtrl = 50000;
            ...
            if (sFTPvar.dsock_state == DATASOCK_READY)
            {
                if (sFTPvar.DataSockConnected_u8 != true) {
                    SockDataState = AppEth_Socket_Control (DATA_SOCK, sFTPvar.RemoteIP, sFTPvar.RemotePort);
                } else {
...
static uint8_t _Cb_ModbTCP_Client(uint8_t event)
{
    ...
    {
        SockCtrlState = AppEth_Socket_Control(
                            SOCKET_MODCLIENT,
                            sModemInfor.sServerModTCP.IPnum,
                            sModemInfor.sServerModTCP.Port_u16);
    ...
    switch(step)
    {
        ...
        case 0:
            ...
            {
                if(getSn_SR(SOCKET_MODCLIENT) != SOCK_ESTABLISHED)
                {
                    sTransModTCP.Flag = FALSE;
                    break;
                }

                ret = send(SOCKET_MODCLIENT, sTransModTCP.aData, sTransModTCP.length);
                if(ret != sTransModTCP.length)
                {
                    UTIL_Printf_Str(DBLEVEL_H,"u_app_eth: ModbusTCP send fail\r\n");
                    
                    sTransModTCP.Flag = FALSE;
                    break;
                }

                UTIL_Printf_Str(DBLEVEL_H, "\r\nu_app_eth: ModbusTCP send:\r\n");
        ...
        case 1:
            ...
            while((size = getSn_RX_RSR(SOCKET_MODCLIENT)) > 0)
            {
                if(size > (DATA_BUF_SIZE - totalLen))
                    size = DATA_BUF_SIZE - totalLen;

                ret = recv(SOCKET_MODCLIENT, &aRECEIVE_MODBUS[totalLen], size);
                ...
                if(totalLen >= DATA_BUF_SIZE)
                    break;
            }

            if(totalLen > 0)
            {
                UTIL_Printf_Str(DBLEVEL_H, "u_app_eth: ModbusTCP recv:\r\n");
                UTIL_Printf_Hex(DBLEVEL_H, aRECEIVE_MODBUS, totalLen);
                UTIL_Printf_Str(DBLEVEL_H,"\r\n");

                memcpy(sDataRecvTCP.Data_a8, aRECEIVE_MODBUS, totalLen);
                sDataRecvTCP.Length_u16 = totalLen;
            ...
            {
                timeoutCnt++;
                if(timeoutCnt >= 5)      // 9 × 100 ms = 500 ms
                {
                    UTIL_Printf_Str(DBLEVEL_H, "u_app_eth: ModbusTCP timeout\r\n");
...
static uint8_t _Cb_Get_Time_NTP (uint8_t event)
{
    ...
    static uint8_t step = 0;
	int32_t ret = -1;
    static uint8_t pIP[4] = {0};
    
    const char* ntp_server_list[] = 
    {
        "time.google.com",          // Uu tiên 1: Google
        "time.cloudflare.com",      // Uu tiên 2: Cloudflare
        "time.windows.com",         // Uu tiên 3: Microsoft
    };
    uint8_t ptr_ntp_sv = 0;
    
    uint32_t ntp_sec = 0;
    ST_TIME_FORMAT sTime_ntp;
    
    static uint32_t LandMark_u32 = 0;
    
    static uint8_t socket_inited = 0;
    static uint8_t ntp_tx[48];
    static uint8_t ntp_rx[48];
    uint8_t peer_ip[4];
    uint16_t peer_port;
    uint8_t addrlen = 0;
    
    if ((sRTC.hour == 10) && (mGetStime_u8 == false)) 
    {
        mGetStime_u8 = true;
        if(sAppEthVar.rTimeNTP_u8 == true)
            sAppEthVar.rTimeNTP_u8 = false;
    }else if (sRTC.hour != 10) {
        mGetStime_u8 = false;
    }
    
    if(sAppEthVar.rTimeNTP_u8 == false)
        step = 0;
    
    if(sAppEthVar.rTimeNTP_u8 != true)
    {
        sAppEthVar.rTimeNTP_u8 = PENDING;
        switch (step)
        {
            case 0:
                DNS_init(gDATABUF);  // SOCK_DNS
            ...
            case 1:
                ...
                if (ret == 1) {
                    char aTEMP[100] = {0};
                    sprintf (aTEMP, "u_app_eth: DNS Success NTP %s: %d.%d.%d.%d\r\n",(uint8_t*)ntp_server_list[ptr_ntp_sv], pIP[0], pIP[1], pIP[2], pIP[3]);
                    UTIL_Printf_Str (DBLEVEL_M,  aTEMP);
                    socket_inited = 0;
                    close(SOCKET_COMM);
            ...
            case 2:
                if(socket_inited == 0)
                {
                    if(socket(SOCKET_COMM, Sn_MR_UDP, AllocatePort(), 0) == SOCKET_COMM)
                        socket_inited = 1;
                }

                if(getSn_SR(SOCKET_COMM) == SOCK_UDP)
                {
                    while (getSn_RX_RSR(SOCKET_COMM) > 0)
                    {
                        recvfrom(SOCKET_COMM, ntp_rx, sizeof(ntp_rx), peer_ip, &peer_port, &addrlen);
                    }
                    step++;
                }
                break;
            ...
            case 3:
                memset(ntp_tx, 0, sizeof(ntp_tx));
                ntp_tx[0] = 0x1B;
                ret = sendto(SOCKET_COMM, ntp_tx, 48, pIP, 123, 4);
                if(ret == 48)
                    step++;
                break;
                ...
                if (getSn_RX_RSR(SOCKET_COMM) >= 48)
                {
                        ...
                        (ntp_rx[1] >= 1) && (ntp_rx[1] <= 15))  // Stratum hop le
                    {
                        ntp_sec =
                            ...
                            ((uint32_t)ntp_rx[41] << 16) |
                            ((uint32_t)ntp_rx[42] << 8 ) |
                            ((uint32_t)ntp_rx[43]);

                        NTP_To_ST_TIME(ntp_sec, &sTime_ntp);
                        if(Modem_Set_sTime(&sTime_ntp, 7) == true)
                        {
                            char aTEMP[64] = {0};
                            sprintf (aTEMP, "u_app_eth: NTP Time: %d/%d/%d|%d:%d:%d +0\r\n", sTime_ntp.year, sTime_ntp.month, sTime_ntp.date, sTime_ntp.hour, sTime_ntp.min, sTime_ntp.sec);
                            UTIL_Printf_Str (DBLEVEL_M,  aTEMP);
        ...
        if (UTIL_var.ModePower_u8 == _POWER_MODE_ONLINE) {
            if (UTIL_var.ModeConnNow_u8 == _CONNECT_FTP_UPLOAD) {
                ...
                if ( ( ( (sMemVar.Status_u8 == ERROR) && (sMemVar.cQueueSwapp_u8 > MEM_MAX_QWRITE_ITEM) ) 
                     ...
                     && (Pending_u8 == false) && (AppSS_iPending() == false) ) {
...
int32_t AppEth_Socket_Control (uint8_t sn, uint8_t* destip, uint16_t destport)
{
...
//                *any_port = 1024;
//            }
            socket(sn, Sn_MR_TCP, AllocatePort(), 0x00);
...
void AppEth_Init_Socket (void)
{
    ...
    if(sEventAppEth[_EVENT_ETH_SOCK_CTRL].e_status == 1)
        fevent_active(sEventAppEth, _EVENT_ETH_DNS_PROCESS);
}

uint16_t AllocatePort(void)
{
    uint8_t  sn = 0;
    uint8_t  port_is_used = 0;
    uint8_t  active_socket_count = 0;
    uint16_t target_port = 0;
    uint16_t used_ports[_WIZCHIP_SOCK_NUM_] = {0};
    uint16_t max_search_attempts = 65535 - 49152;   //So port su dung

    for(sn = 0; sn < _WIZCHIP_SOCK_NUM_; sn++)
    {
        if(getSn_SR(sn) != SOCK_CLOSED)
        {
            used_ports[active_socket_count++] = getSn_PORTR(sn);
        }
    }

    if(active_socket_count == 0)
    {
        target_port = sAppEthVar.AnyPortCtrl++;
        if((sAppEthVar.AnyPortCtrl > 65534) || (sAppEthVar.AnyPortCtrl < 49152))
            sAppEthVar.AnyPortCtrl = 49152;
        
        return target_port;
    }

    while(max_search_attempts--)
    {
        target_port = sAppEthVar.AnyPortCtrl++;
        if((sAppEthVar.AnyPortCtrl > 65534) || (sAppEthVar.AnyPortCtrl < 49152))
            sAppEthVar.AnyPortCtrl = 49152;
        ...
        for(sn = 0; sn < active_socket_count; sn++)
        {
            if(used_ports[sn] == target_port)
            {
                port_is_used = 1;
                break;
            }
