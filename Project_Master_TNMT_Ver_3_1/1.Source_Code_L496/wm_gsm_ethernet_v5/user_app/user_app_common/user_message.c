...
void mInit_Header_Subcribe (void)
{
    ...
    UTIL_MEM_set(sMessage.aSubcribe_Header, 0, sizeof(sMessage.aSubcribe_Header));
	sMessage.sSubcribe.Data_a8 = sMessage.aSubcribe_Header;
...
{
#ifdef PACKET_MQTT_PROTOCOL
    UTIL_MEM_set(sMessage.aCONNECT, 0, sizeof(sMessage.aCONNECT));
	sMessage.sConnect.Data_a8 = sMessage.aCONNECT;
	sMessage.sConnect.Length_u16 = 0;
	...
	sMessage.sConnect.Length_u16 = MQTTSerialize_connect(sMessage.sConnect.Data_a8, sizeof(sMessage.aCONNECT), &Connect_Packet);
#endif
}
...
{ 
    ...
    uint8_t Max_Packet_In_Mess = 1;
    
    //danh dau lai mess moi
    switch (tdata)
    {
        case _MEM_DATA_TSVH:
        #if (TSVH_OPERA_OR_FLASH == TSVH_OPERA)
            tmess = DATA_TSVH_OPERA;
        #elif (TSVH_OPERA_OR_FLASH == TSVH_FLASH)
            tmess = DATA_TSVH_FLASH;
        #else
            tmess = DATA_TSVH_MULTI;
        #endif
            pRec = &sRecTSVH;
            break;
    ...
    if (type == 0) {
        //xoa danh dau cac mess truoc do
        mReset_Raw_Data();
        ...
        if ( (sMemVar.nSending_u16 < Max_Packet_In_Mess) 
            ...
            UTIL_MEM_cpy(sMessage.pRawData.Data_a8 + sMessage.pRawData.Length_u16, pData, Length);
            sMessage.pRawData.Length_u16 += Length;
    ...
    if (tmess != DATA_GPS) {
        //tinh crc tong
        sMessage.pRawData.Length_u16++;
        for (uint16_t i = 0; i < (sMessage.pRawData.Length_u16 - 1); i++)
            TempCrc ^= sMessage.pRawData.Data_a8[i];
