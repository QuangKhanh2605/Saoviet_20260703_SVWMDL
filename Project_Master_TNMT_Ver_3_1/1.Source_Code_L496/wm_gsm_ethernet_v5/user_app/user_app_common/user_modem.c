...
sModemInformation sModemInfor = 
{
    ...
    .sServerBack.IPnum = {117, 6, 163, 177},
    .sServerBack.Port_u16 = 1883,
    
    .sServerBack.KeepAlive_u32 = 60,
    .sServerBack.DomainOrIp_u8 = __SERVER_IP, 
        
    //Server ModbusTCP
    .sServerModTCP.aIP        = {"0.0.0.0"},              //Khoi dong khong dung
    .sServerModTCP.aPORT      = {"502"},
    .sServerModTCP.aUSER      = {"admin"},
    .sServerModTCP.aPASS      = {"admin"},
    
    .sServerModTCP.IPnum = {0, 0, 0, 0},
    .sServerModTCP.Port_u16 = 502,
};
...
void Modem_Init_Var (void)
{
    ...
    if (temp != FLASH_BYTE_EMPTY) {
        OnchipFlashReadData(ADDR_MODEM_INFOR, &aBuff[0], 2048);
...
void Modem_Save_Var (void)
{
    ...
    OnchipFlashWriteData(ADDR_MODEM_INFOR, &aBuff[0], 2048);
#endif
}
...
{   
    ...
    {
        //Convert lai IP       
        if (UTIL_Convert_IP_To_Buff(pUpdate->sServer.aIP, pUpdate->sServer.IPnum) == true) {
            pUpdate->sServer.DomainOrIp_u8 = __SERVER_IP;
        } else {
            pUpdate->sServer.DomainOrIp_u8 = __SERVER_DOMAIN;
        }
        pUpdate->sServer.Port_u16 = (uint16_t) UtilStringToInt(pUpdate->sServer.aPORT);
