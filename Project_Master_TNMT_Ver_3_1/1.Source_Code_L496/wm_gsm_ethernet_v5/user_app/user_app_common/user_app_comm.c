...
volatile uint8_t iUartError_u8;
...
static uint8_t _Cb_Uart_Rx_Debug(uint8_t event)
{
    ...
    if (MarkFirstRecvUart == 0)
    {
        ...
        if (sUartDebug.Length_u16 == LastLengthRecv)
        {
            ...
            Modem_Check_AT(&sUartDebug, _UART_DEBUG);
...
static uint8_t _Cb_Uart_Error(uint8_t event)
{
    static uint16_t cError = 0;
    
    APP_PRINTF("u_app_com: uart %d error!\r\n", iUartError_u8 );
    
    if (iUartError_u8 == _UART_DEBUG)
    {
...
static uint8_t _Cb_HandLer_IDLE(uint8_t event)
{
    ...
    if ((sRTC.hour == 9) && (mGetStime_u8 == false)) {
        mGetStime_u8 = true;
...
static uint8_t _Cb_Tx_Timer(uint8_t event)
{
    if (sMemVar.Status_u8 == ERROR) {
        fevent_active( sEventMem, _EVENT_MEM_TEST_WRITE);
        Modem_Packet_Alarm_String("u_app_comm: memory error\r\n");
    }

    if (AppComm_Set_Next_TxTimer() == false) {
        return false;
    }
    ...
    {
        if ( (sRTC.hour == 0) && (sRTC.min < 2) ) {
            //den 0h00 thi quy dinh lai gio tron
            sModemVar.cTSVH_u8 = sModemInfor.sFrequence.nTSVH_u8; 
        }
        
        sModemVar.cTSVH_u8++;
        if (sModemVar.cTSVH_u8 >= sModemInfor.sFrequence.nTSVH_u8)
        {
            sModemVar.cTSVH_u8 = 0;
        ...
        #ifdef SERVER_TNMT  //doi ve mode FTP upload
            UTIL_var.ModeConnFuture_u8 = _CONNECT_FTP_UPLOAD; 
    ...
    fevent_enable(sEventAppWM, _EVENT_ENTRY_WM);
#endif
    
#ifdef USING_APP_SENSOR
    fevent_enable(sEventAppSensor, _EVENT_SENSOR_LOG_TSVH);
#endif
    
    Get_RTC();
    sModemVar.LandMarkOpera_u32 = HW_RTC_GetCalendarValue_Second(sRTC, 0);
...
void SysApp_Setting (void)
{
	...
	UTIL_TIMER_SetPeriod (&TimerTx, 60000);  
...
void AppComm_Init (void)
{
    ...
    pFunc_UTIL_Log_To_Mem = &AppComm_Save_Log; 
    ...
    if (Count >= 6) {
        sModemVar.DetHardware_u8 = __HARD_ONLINE_ONLY;
        UTIL_var.ModePower_u8   = _POWER_MODE_ONLINE;
        
        UTIL_Printf_Str(DBLEVEL_M, "u_app_com: hardware online only\r\n" );
    } else {
        sModemVar.DetHardware_u8 = __HARD_ONLINE_PSM;
...
{    
    ...
    //Sub second current, and add to 6s 
    if (sRTC.sec < (FreqCacul * 60))
    {
        FreqCacul  = FreqCacul * 60 - sRTC.sec + 10;
...
uint8_t AppComm_Set_Next_TxTimer (void)
{
    ...
    {
        sModemInfor.sFrequence.UnitMin_u16 = sModemInfor.sFrequence.nTSVH_u8 * sModemInfor.sFrequence.Duration_u32;
        //cacul period again to send at rounding stime:
        FreqCacul = AppCom_Calcu_Period_To_RoudTime (sModemInfor.sFrequence.Duration_u32);
    }
    
    //thoi gian qua ngan thi se return 0
    if (FreqCacul < 30000) {
        result = false;
        FreqCacul = 30000;
    }
    ...
    //Set tiep timer Tx 
	if (UTIL_TIMER_Start(&TimerTx) != UTIL_TIMER_OK) {
        UTIL_TIMER_Start(&TimerTx);
    }
...
void AppComm_DBG_Rx_Callback (uint16_t Size)
{
    ...
    *(sUartDebug.Data_a8 + sUartDebug.Length_u16++) = UartDebugRxByte;
    ...
    if (uart_dma_debug.Init.Mode == DMA_NORMAL)   //sua o day
    {
        ...
        UTIL_MEM_cpy(sUartDebug.Data_a8 + sUartDebug.Length_u16, aUART_DBG_DMA_RX, Size);
    ...
    } else
    {
        ...
        if (Size > OldPosData)
        {
            ...
            UTIL_MEM_cpy(sUartDebug.Data_a8 + sUartDebug.Length_u16, &aUART_DBG_DMA_RX[OldPosData], NbByteRec);
...
//        pFunc_UTIL_Debug_Send(p_data, size);
    UTIL_Printf(DBLEVEL_M,  p_data, size);
}
...
uint8_t AppComm_Save_Log (uint8_t *pData, uint16_t Length)
{
    ...
    sprintf((char *) aTEMP, "%d.%d/%d %d:%d:%d.", sRecLog.iSave_u16, sRTC.month, sRTC.date, 
    ...
    Mem_Write_Data(sMemVar.Type_u8, _MEM_DATA_LOG, 0,
                      &aTEMP[0], Count, sRecLog.Size_u16);
